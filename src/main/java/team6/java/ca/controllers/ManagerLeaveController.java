...
public class ManagerLeaveController {
    ...
    public String managerApproveLeave(@PathVariable("id") Long leaveId) {
        ...
        return "redirect:/manager/check/" + leaveRecord.getApproveManager().getUserId();  
    }
    ...
    public String viewMyTeam(@PathVariable("managerId") Long id, @RequestParam(value = "dateDropdown", required = false) String startDate, Model model) {
        ...
        List<EmployeeLeaveRecord> targetEmployeeLeaveRecords = lrService.findAllByLeaveDateBetweenAndApproveMgrId(today, endOfMonth, id);
        ...
        List<EmployeeLeaveRecord> approvedLeaveRecordsUnderCurrentManager = lrService.findApprovedLeaveRecordByManagerEmpId(id);
        if (approvedLeaveRecordsUnderCurrentManager == null) {
            approvedLeaveRecordsUnderCurrentManager = new ArrayList<EmployeeLeaveRecord>();
        }

        List<EmployeeLeaveRecord> pendingLeaveRecordsUnderCurrentManager = lrService.findPendingLeaveRecordByManagerEmpId(id);
        ...
        for (LeaveType leaveType : leaveTypes) {
            List<EmployeeLeaveRecord> leaveRecords = lrService.findAllLeaveRecordsOfLeaveTypeUnderManager(leaveType.getLeaveTypeId(), id);
            ...
            for (EmployeeLeaveRecord record : leaveRecords) {
                ...
                int weekendDays = lutilService.getNumberOfWeekendDaysInLeaveRange(record.getLeaveDate(), record.getEndDate());
                int publicHolidays = pService.getNumberOfPublicHolidaysInRange(record.getLeaveDate(), record.getEndDate());
