...
public interface EmployeeLeaveRecordRepository extends JpaRepository<EmployeeLeaveRecord, Long> {
    ...
    List<EmployeeLeaveRecord> findAllByLeaveDateBetween(@Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);
    ...
    List<EmployeeLeaveRecord> findAllLeaveRecordsOfLeaveTypeUnderManager(@Param("leaveTypeId") Long leaveTypeId,@Param("mgrId") Long managerId);
    ...
    List<EmployeeLeaveRecord> findAllByLeaveDateBetweenAndApproveMgrId(@Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate, @Param("mgrId") Long managerId);
    
    
    //for csv file generate
    
    @Query("SELECT e FROM EmployeeLeaveRecord e WHERE e.leaveDate = CURRENT_DATE")
    List<EmployeeLeaveRecord> findAllCurrentLeaveRecords();

    @Query("SELECT e FROM EmployeeLeaveRecord e WHERE e.approveManager.userId = :managerId AND e.leaveDate = CURRENT_DATE")
    List<EmployeeLeaveRecord> findAllCurrentLeaveRecordsUnderManager(@Param("managerId") Long managerId);
}
